...
do
        for numLists in 4, 8, 16
        do
                ./lab2_list --iterations=1000 --threads=$numThreads --sync=m --lists=$numLists >> lab2b_list.csv
                ./lab2_list --iterations=1000 --threads=$numThreads --sync=s --lists=$numLists >> lab2b_list.csv
        done
done
